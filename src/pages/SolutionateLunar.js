...
const SolutiiLunarPage = () => {
  const { solutii, arestati } = useLoaderData();
  const [solutiiVizibile, setSolutiiVizibile] = useState(true);

  return (
    <>
      <button onClick={() => setSolutiiVizibile(!solutiiVizibile)} style={{ zIndex:"1", position: "relative", marginBottom: "-64px", borderRadius: "6px", padding: "6px", backgroundColor: "#f6ad1b", color:"white ", verticalAlign: "bottom", fontSize: "20px", marginLeft: "120px" }}>
        {solutiiVizibile ? "arestati" : "solutii"}
      </button>
      <div style={{ display: "flex" }}>
        {!solutiiVizibile && <Suspense fallback={<p style={{ textAlign: "center" }}>Loading...</p>}>
